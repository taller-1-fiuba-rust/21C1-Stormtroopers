...
fn obtain_str_command(msg: &str) -> Result<ParsedMessage, ParseError> {
    ...
    let arguments = split_msg.fold(String::new(), |acc, x| if acc.is_empty() {String::from(x)} else {format!("{}{}{}", acc, " ", x)});
    
    return Ok(ParsedMessage{command: command, arguments: arguments});    
}
...
mod tests {
    ...
    fn test_parse_command_and_several_arguments() {
        ...
        let parsed_msg = result_msg.unwrap();
        assert_eq!(parsed_msg.command, "test_command");
        assert_eq!(parsed_msg.arguments, "test_argument_1 test_argument_2 test_argument_3");
    }
